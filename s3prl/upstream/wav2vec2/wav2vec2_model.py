...
NORMALIZATION_CHOICES = ChoiceEnum(["layer_norm", "batch_norm"])
...
class ConvolutionModule(torch.nn.Module):
    ...
    def __init__(
        ...
        self.encoder_cnn_norm = encoder_cnn_norm
        if(encoder_cnn_norm=="batch_norm"):
            self.batch_norm = torch.nn.BatchNorm1d(channels)
        else:
            self.batch_norm = LayerNorm([channels], export=export)  #for convenience, we don't change the name of this module
    ...
    def forward(self, x):
        ...
        if (self.encoder_cnn_norm == 'layer_norm'):
            x = x.transpose(1, 2)
        x = self.batch_norm(x)
        if (self.encoder_cnn_norm == 'layer_norm'):
            x = x.transpose(1, 2)
...
class ConformerEncoderLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_module = ConvolutionModule(
            ...
            encoder_cnn_norm=encoder_cnn_norm,
        )
