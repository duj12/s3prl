...
def get_pretrain_args():
    ...
    if args.past_exp:
        ...
        if args.expdir != ckpt['Args'].expdir:
            ckpt['Args'].expdir = args.expdir
        args = update_args(args, ckpt['Args'])
        os.makedirs(args.expdir, exist_ok=True)
        args.init_ckpt = ckpt_pth
        config = ckpt['Config']
        with open(args.config, 'r') as file:
            ori_config = yaml.load(file, Loader=yaml.FullLoader)
            if 'pretrain_expert' not in config:
                config['pretrain_expert'] = ori_config['pretrain_expert']
            if 'runner' not in config:
                config['runner'] = ori_config['runner']
            if 'optimizer' not in config:
                config['optimizer'] = ori_config['optimizer']
    else:
